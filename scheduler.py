...
def crawl_range(start: str, end: str, source_key: str = 'CNY') -> dict:
    """
    抓取指定区间数据入库（仅 CNY 支持区间，IDR/HKD 只抓 end 当天）
    """
    cfg = SOURCES.get(source_key)
    if not cfg:
        return {'status': 'failed', 'message': f'未知数据源: {source_key}'}
...
def backfill_recent(days: int = 30, source_key: str = 'CNY') -> dict:
    ...
    missing = database.get_missing_dates(start.isoformat(), end.isoformat(), base='CNY')
    if not missing:
        return {'status': 'success', 'inserted': 0, 'skipped': 0, 'message': '无缺失日期'}

    logger.info(f'[CNY] 回填 {len(missing)} 个缺失日期: {missing[:5]}...')
    total_ins, total_skip = 0, 0
    for ds in missing:
        try:
            rows = scraper.fetch(ds, ds)
            if rows:
                ins, skp = database.upsert_many(rows, base='CNY')
                total_ins += ins
                total_skip += skp
        except Exception as e:
            logger.error(f'[CNY] 回填 {ds} 失败: {e}')
    return {
        'status': 'success',
        'inserted': total_ins,
        'skipped': total_skip,
